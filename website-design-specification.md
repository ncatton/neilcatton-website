...
4. **Minimum viable data collection.** No cookie, script, or form field exists unless it earns its place under Section 9. The site currently has one real tracking script (Google Analytics) and two third-party embeds (Microsoft Bookings, Microsoft 365 (Graph)) — keep it that lean.
...
- **Keyboard operability.** Every interactive element — nav dropdown, the contact form, the Microsoft Bookings embed, the search overlay, FAQ accordions — fully usable with keyboard alone, with a visible focus indicator at all times. WCAG 2.2's **Focus Not Obscured** criterion means the fixed nav bar must never cover a focused element lower on the page.
...
- **Categorise honestly.** Strictly Necessary (Microsoft 365 (Graph) session, security), Functional (search/cookie-choice memory, if any), Analytics (GA), Marketing (none currently — say so plainly rather than including a boilerplate category that doesn't apply).
...
- **Third-party embeds.** Microsoft Bookings and Microsoft 365 (Graph) both set their own cookies/session state when embedded or submitted to — list them explicitly in the privacy policy (check whether `privacy.html` already does — confirm and update if not).
...
- **Privacy policy** — exists (`privacy.html`); confirm it names every third-party processor (Microsoft 365 (Graph), Microsoft Bookings, Google Analytics) and the legal basis for each.
...
- Reserve explicit `width`/`height` on every image and embed (including the Microsoft Bookings iframe and the Substack embed) to prevent layout shift — already done for the images checked; confirm on the Microsoft Bookings and Substack iframes specifically.
...
- A Content Security Policy restricting script/style sources to known origins (self, Google Fonts, Microsoft Bookings, Microsoft 365 (Graph), Google Analytics, Substack) — not currently set in `netlify.toml`; a real gap, but one that needs careful testing against every embed before going live, since a misconfigured CSP breaks the Microsoft Bookings and Substack iframes silently (Section 19).
- Form spam protection on the contact form beyond Microsoft 365 (Graph)'s own filtering — a honeypot field is the lower-effort, privacy-respecting option (avoids adding a Google reCAPTCHA tracker for a low-value anti-spam gain).
...
- Track meaningful outcomes: contact form submissions, Microsoft Bookings booking clicks, article read-throughs — not vanity pageview totals.
...
- The homepage's own contact form (`index.html`) submitted to `action="#"` with no JavaScript handler — a genuine dead end matching this document's own Section 1, Principle 6. Wired to the same Microsoft 365 (Graph) endpoint and confirmation pattern already working correctly on `contact.html`.
...
2. **Test the new CSP on the next deploy.** Check the browser console on the homepage, a service page, and the contact page for anything blocked — the Microsoft Bookings link, the Substack embed, and the feed-fallback fetches are the most likely to be affected by a too-tight policy.
